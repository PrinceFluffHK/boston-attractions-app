...
class ReviewSeeder {
    static async seed() {
        ...
        //reviews
        await Review.query().insert({ siteId: bostonCommon.id, userId: garrett.id, rating: 5, 
            reviewBody: "Lots of sunlight, very cool sculpture, love the coding camp down the street!"})
        await Review.query().insert({ siteId: faneuilHall.id, userId: matthew.id, rating: 1, 
            reviewBody: "Terrible historical site, 0 dead bodies" })
        await Review.query().insert({ siteId: faneuilHall.id, userId: garrett.id, rating: 2 })
    }
}
